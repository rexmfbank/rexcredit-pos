...
import 'package:rex_app/src/modules/shared/spend/bill_payment/bill_cable/states/cable_screen_state.dart';
import 'package:rex_app/src/modules/shared/spend/bill_payment/bill_cable/widgets/select_bouquet_modal_sheet.dart';
...
import 'package:rex_app/src/utils/debouncer.dart';
...
class CableTvNotifier extends StateNotifier<CableScreenState> with LocatorMix {
  ...
  void filterCableBillers(String query) {
    if (state.selectedServiceProvider != null &&
      ...
      state = state.copyWith(
        cableBillers: filteredList,
      );
    }
  }

  void setSelectedBouquetPlan(
      BuildContext context, BillerProduct billerProduct) {
    state = state.copyWith(
  ...
  void fillBeneficiaryInfo(
      {required String cardNumber, required String billerCode}) {
    state = state.copyWith(
  ...
  Future<void> validateSmartCardNo(BuildContext context) async {
    ...
    try {
      ...
      state = state.copyWith(
          validateSmartCardNoLoading: false, apiError: error.toString());
      if (context.mounted) {
