...
import 'package:rex_app/src/modules/revamp/dashboard/models/home_transfer_state.dart';
import 'package:rex_app/src/modules/revamp/dashboard/providers/dashboard_providers.dart';
import 'package:rex_app/src/modules/revamp/dashboard/ui/transfer/widgets/select_beneficiary_list.dart';
...
class HomeTransferNotifier extends AutoDisposeNotifier<HomeTransferState>
      ...
      child: SelectBeneficiaryList(
        onClick: (beneficiaryData) {
          state = state.copyWith(
            ...
            bankCode: beneficiaryData.finEntityCode,
            bankName: beneficiaryData.finEntityName,
            bankNameController: TextEditingController(
              text: beneficiaryData.finEntityName,
            ),
            accountNumberController: TextEditingController(
              text: beneficiaryData.beneficiaryAccount,
            ),
          );
          context.pop();
        },
  ...
  Future<void> getListOfBanks(BuildContext context) async {
    ...
    try {
      final res = await RexApi.instance.getBankList(
        authToken: ref.read(appAuthTokenProvider) ?? '',
      );
      state = state.copyWith(isLoading: false, bankList: res, banks: res.data);
    } catch (error, _) {
  ...
  void updateBankInfo(String value) {
    ...
    state = state.copyWith(bankSelected: false);
  }
  ...
  Future<void> searchBeneficiaries({required BuildContext context}) async {
    state = state.copyWith(isSearching: true, beneficiaries: []);
    try {
      ...
      widget: TransactionPinSheet(
        ...
        onPinComplete: (v) {
          ...
          if (state.saveBeneficiary) {
            saveTransactionBeneficiary(context, v);
            return;
          }
          makeInterBankTransfer(context, v);
        },
      ),
    );
      ...
      final request = InterBankTransferRequest(
        ...
        geolocation:
            location.currentPosition != null
                ...
                : '',
        amount: double.parse(state.amountController.text.removeCommas()),
...
final fetchBeneficiaryProvider = FutureProvider<List<BeneficiaryData>?>((
  ref,
) async {
